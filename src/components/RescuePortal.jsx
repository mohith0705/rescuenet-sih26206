    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs text-slate-500 font-medium">Pending SOS Calls</div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs text-slate-500 font-medium">Critical Priority Victims</div>
            <div className="text-2xl font-extrabold text-amber-600 font-mono mt-1">{criticalCount}</div>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs text-slate-500 font-medium">Active NDRF Rescue Teams</div>
            <div className="text-2xl font-extrabold text-blue-600 font-mono mt-1">{rescueTeams.length}</div>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
          <div>
            <div className="text-xs text-slate-500 font-medium">Successfully Evacuated</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 p-4 flex flex-col space-y-4 h-[620px] shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-900 text-sm flex items-center gap-1.5">
                <Radio className="w-4 h-4 text-red-600 animate-pulse" /> Live SOS Feed
              </h3>
              <p className="text-[11px] text-slate-500">Real-time distress signals queue</p>
            </div>
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 text-[11px]">
              <button 
                ...
                className={`px-2 py-1 rounded font-semibold transition ${sosFilter === 'PENDING' ? 'bg-red-600 text-white shadow-sm' : 'text-slate-600'}`}
              >
                Pending
              </button>
              <button 
                onClick={() => setSosFilter('DISPATCHED')} 
                className={`px-2 py-1 rounded font-semibold transition ${sosFilter === 'DISPATCHED' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600'}`}
              >
                Active
              </button>
              <button 
                onClick={() => setSosFilter('ALL')} 
                className={`px-2 py-1 rounded font-semibold transition ${sosFilter === 'ALL' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-600'}`}
              >
                All
              </button>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border cursor-pointer transition-all space-y-2 ${
                      isSelected
                        ? 'bg-red-50/60 border-red-400 shadow-md ring-1 ring-red-400'
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-600 flex items-center gap-1 mt-0.5 truncate max-w-[200px]">
                          <MapPin className="w-3 h-3 text-slate-400 shrink-0" /> {sos.location}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <span className="text-xs font-mono font-bold text-amber-700">{sos.peopleCount} Trapped</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-[11px] pt-2 border-t border-slate-200/60">
                      ...
                      {sos.assignedTeam && (
                        <span className="text-slate-500 text-[10px] font-mono truncate max-w-[120px]">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-slate-500">ID: #{selectedSos.id}</span>
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 mt-1">{selectedSos.name}</h3>
                </div>

                <div className="flex items-center gap-2">
                  <a
                    href={`tel:${selectedSos.phone}`}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-900 border border-slate-200 text-xs font-bold px-3 py-2 rounded-lg flex items-center gap-1.5 transition"
                  >
                    <Phone className="w-3.5 h-3.5 text-emerald-600" /> Call Victim ({selectedSos.phone})
                  </a>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 space-y-2">
                  <div className="text-slate-500 font-medium">Victim Location & Landmark:</div>
                ...
                <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 space-y-2">
                  <div className="text-slate-500 font-medium">Incident Notes & Needs:</div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-600 font-medium">Assign Rescue Team:</span>
                  <select
                    value={selectedSos.assignedTeam || ''}
                    onChange={(e) => onAssignTeam(selectedSos.id, e.target.value)}
                    className="bg-white border border-slate-300 text-xs text-slate-900 rounded-lg p-2 focus:outline-none focus:border-blue-500 font-medium"
                  >
                ...
                <div className="flex items-center gap-2">
                  {selectedSos.status === 'PENDING' && (
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-md transition"
                    >
                  ...
                  {selectedSos.status === 'DISPATCHED' && (
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-md transition"
                    >
